  ...
  <section class="card" @click="switchShopPage">
    <div class="card-body">
      ...
      <div class="logo_container">
        ...
        <img
          class="brand-img"
          v-if="shop.is_premium"
          :src="require('@/assets/品牌.png')"
          alt
        />
      </div>
      <!-- right  -->
      <div class="card-main">
        ...
        <div class="second-row">
          <div class>
            <span class="rating">
              <b class="rating-bold">{{ shop.rating }}</b
              >分
            </span>
        ...
        <div class="third-row">
          <div class>
            <span class="mr-1"
              >起送￥{{ shop.float_minimum_order_amount }}</span
            >
...
export default {
  ...
  data() {
    return {
      ...
      tags: ["28减10", "45减16", "60减19", "35减4", "40减7", "23减2", "21减6"],
    };
  },
  computed: {
    cartNum() {
  ...
  methods: {
    ...
    getCartLength() {
      ...
      if (carts.length === 0) {
        ...
        const cart = carts.find((cart) => cart.id == this.shop.id);
        if (cart) {
