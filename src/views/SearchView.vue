<template>
    <div class="search">
        <van-sticky>
            <search-bar @confirm-search="confirm" @clear-val="clearInput" @go-back="switchPageBack" class="search"
                v-model="inputVal" :placeholder="placeholderPassed" ref="searchBarFixed"></search-bar>
        </van-sticky>

        <div v-if="!isConfirmed">
            <search-suggest v-if="!inputVal.trim()"></search-suggest>
            <search-results @select-shop="confirm" v-else :shopResults="restaurants" :wordResults="words">
            </search-results>
        </div>
        ...
        <div v-else>
            ...
            <div v-else>
                <van-sticky :offset-top="44">
                    <filter-bar class="filter-bar-background" @get-condition="setCondition" :fixedOn="false">
                    </filter-bar>
                </van-sticky>
...
export default {
    ...
    methods: {
        ...
        async confirm(payload = this.inputVal) {
            ...
            this.$router.push({ name: 'search', query: { keyword: this.inputVal.trim() } })
            /* eslint-disable */
            try {
                const res = await this.$axios(`/api/profile/typeahead/${this.inputVal.trim()}`)
        ...
        debouncedSearch: debounce(
            async function () {
                ...
                if (this.inputVal.trim()) {
                    try {
                        const res = await this.$axios(`/api/profile/typeahead/${this.inputVal}`)
